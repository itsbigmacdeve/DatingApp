...
namespace API.SignalR
{
    ...
    public class MessageHub :Hub
    {
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            var messages = await _uow._messageRepository.GetMessageThread(Context.User.GetUsername(), otherUser);
        ...
        {
            var group = await _uow._messageRepository.GetMessageGroup(groupName);
            ...
            if (group == null)
            {
                ...
                _uow._messageRepository.AddGroup(group);
            }
        ...
        {
            var group = await _uow._messageRepository.GetGroupForConnection(Context.ConnectionId);
            var connection = group.Connections.FirstOrDefault(x => x.ConnectionId == Context.ConnectionId);
            _uow._messageRepository.RemoveConnection(connection);
            if (await _uow.Complete()) return group;
